<template>
  <div id="banner-list">
    ...
    <el-dialog title="编辑问题" :visible.sync="dialogVisible" width="70%">
...
<script>
import { takeDiningNaire, createQuestion, delQuestionById } from "@/api/dining";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    editHandle() {
      createQuestion({ ...this.exer, naireId: this.naireId, type: this.status, id: this.exerId }).then(res => {
        this.dialogVisible = false;
        this.$message({
          message: "修改成功",
          type: "success"
    ...
    delHandle(_data) {
      console.log(_data);
      this.$confirm('此操作将永久删除数据且无法恢复, 是否继续?', '提示', {
